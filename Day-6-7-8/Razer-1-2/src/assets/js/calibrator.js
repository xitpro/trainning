...
class Calibrator {
	...
	drawRing() {
		...
		var glow = ctx.createRadialGradient(this.opt.canvasW/2, this.opt.canvasW/2, this.opt.ringRadius/2, this.opt.canvasW/2, this.opt.canvasW/2, this.opt.ringRadius);
		...
		ctx.fillStyle = glow;
		ctx.beginPath();
		ctx.arc(this.opt.canvasW/2, this.opt.canvasW/2, this.opt.ringRadius, 0, fullRadian(), false);
		ctx.fill();
	...
	drawPoint(index=0, d=this.opt.dynamData[0], color='#666') {
		...
		for(var x=1;x<=13;x++) {
			...
			if((this.eventPoint >= 0 && index == this.eventPoint && this.toggleAnimation)
				||
				(this.activeIndex >= 0 && index == this.activeIndex && this.toggleAnimation)) {
			...
			ctx.strokeStyle = waveColor;
			ctx.arc(20, 0, this.opt.pointRadius+20+this.opt.waveGap*x, degreeToRadian(-180-10), degreeToRadian(-180+10), false);
			ctx.stroke();
		}
	}
	...
	getEventPoint(e) {
		...
		if(Math.pow(x,2)+Math.pow(y,2)<=Math.pow((this.opt.ringRadius-this.opt.pointRadius),2)) {

			this.dom.style.cursor = 'default';

		} else if(Math.pow(x,2)+Math.pow(y,2)<=Math.pow((this.opt.ringRadius+this.opt.pointRadius),2)) {

			for(var i=0;i<=6;i++) {

				var pointEdgeY = Math.floor(Math.sin(degreeToRadian(this.opt.dynamData[i]))*this.opt.ringRadius);
				pointEdgeY = -pointEdgeY;
				var pointEdgeX = Math.floor(Math.cos(degreeToRadian(this.opt.dynamData[i]))*this.opt.ringRadius);

				if(isBetween(x, pointEdgeX-this.opt.pointRadius, pointEdgeX+this.opt.pointRadius)
					 && isBetween(y, pointEdgeY-this.opt.pointRadius, pointEdgeY+this.opt.pointRadius)) {
					hasPoint = i;
				}
			}
	...
	getEventIndex(e) {
		...
		if(Math.pow(x,2)+Math.pow(y,2) <= Math.pow((this.opt.ringRadius-this.opt.pointRadius),2)) {
			...
			for(var i=0;i<=6;i++) {
				...
				if(i != 4) {
					if(eventDegree <= this.opt.dataRange[i].max
						 &&  eventDegree >= this.opt.dataRange[i].min) {
						hoverArea = i;
					}
				} else {
					if(eventDegree < 0 && eventDegree <= this.opt.dataRange[i].max) { hoverArea = i; }
					if(eventDegree > 0 && eventDegree >= this.opt.dataRange[i].min) { hoverArea = i; }
				}
			}
		}
		return hoverArea;
	...
	slicePie(hoverArea) {
		...
		ctx.arc(this.opt.canvasW/2, this.opt.canvasW/2, this.opt.ringRadius-this.opt.ringWeight, degreeToRadian(this.opt.dataRange[this.hoverArea].min), degreeToRadian(this.opt.dataRange[this.hoverArea].max), false);
	...
	animateWave() {
		if(this.eventPoint >= 0 && this.toggleAnimation) {
			this.clearCanvas();
			this.updatePoint(this.eventPoint, this.opt.dynamData[this.eventPoint], mainColor);
			window.requestAnimationFrame(animateOn);
		} else if(this.activeIndex >= 0 && this.toggleAnimation) {
			this.clearCanvas();
			this.updatePoint(this.activeIndex, this.opt.dynamData[this.activeIndex], mainColor);
			window.requestAnimationFrame(animateOn);
		}
	}
	controlRange(index,d) {
		if(index != 4) {
			if(d >= this.opt.dataRange[index].max) { d = this.opt.dataRange[index].max; }
			if(d <= this.opt.dataRange[index].min) { d = this.opt.dataRange[index].min; }

		} else {
			if(d == 0) { d = -180; }
			if(d < 0 && d >= this.opt.dataRange[index].max) { d = this.opt.dataRange[index].max; }
			if(d > 0 && d <= this.opt.dataRange[index].min) { d = this.opt.dataRange[index].min; }
		}
		return d;
	}
	spitData() {
		var data = [];
		for (var i = 0; i < 7; i++) {
			data[i] = (this.opt.dynamData[i]+90 >= 180)? 
				this.opt.dynamData[i]+90-360 : this.opt.dynamData[i]+90;
		}
		return data;
	}
	printData() {
	...
	handleMouseMove(e) {
	    ...
	    if(mouseIsDown && this.eventPoint >= 0) { this.togglePie = false; }
...
function stepperClick(e) {
	...
	if(target.classList.contains('up')) { value++; }
	if(target.classList.contains('down')) { value--; }
