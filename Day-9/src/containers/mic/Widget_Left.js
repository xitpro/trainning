...
class Widget_Left extends Component {
  ...
  render() {
    ...
    if (this.state.isToggleMicrophone) {
      microphone = (
        <div
          ...
          id="swPhone"
          onClick={this.handleActiveMicrophone}
        >
      ...
      activeMicrophone = (
        <div className="slider-container on" id="slPhone">
          ...
          <div
            id="slPhoneFill"
            className="left"
            style={{ width: (this.state.microphoneValue * 520) / 100 }}
          />
          <div className="track" />
          <div
            id="slPhoneTip"
            className="slider-tip"
            style={{ left: (this.state.microphoneValue * 520) / 100 - 15 }}
          >
            {this.state.microphoneValue}
          </div>
          <input
            type="range"
            min={0}
    ...
    if (this.state.isToggleSensitivity) {
      sensitivity = (
        <div
          className="switch on switch-slider "
          id="swSensi"
          onClick={this.handleActiveSensitivity}
        >
          <div className="handle" />
        </div>
      );
      ...
      activeSensitivity = (
        <div className="slider-container on" id="slSensi">
          ...
          <div
            id="slPhoneFill"
            className="left"
            style={{ width: (this.state.sensitivityValue * 520) / 100 }}
          />
          <div className="track" />
          <div
            id="slPhoneTip"
            className="slider-tip"
            style={{ left: (this.state.sensitivityValue * 520) / 100 - 15 }}
          >
            {this.state.sensitivityValue}
          </div>
          <input
            type="range"
            min={0}
