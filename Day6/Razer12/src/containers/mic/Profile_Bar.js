...
class Profile_Bar extends Component {
  ...
  handleClickout = e => {
    if (
      this.state.isDropdownProfilesExpand &&
      e.target.id !== 'profileDropOpt' &&
      (isNaN(e.target.id) || e.target.id.length === 0)
    ) {
      this.setState({
        isDropdownProfilesExpand: false,
      });
    ...
    if (
      this.state.isDropdownMenuExpand &&
      e.target.className !== 'act action' &&
      (isNaN(e.target.id) || e.target.id.length === 0)
    ) {
      this.setState({
        isDropdownMenuExpand: false,
    ...
    if (
      this.state.isDeleteProfileToggle &&
      e.target.id !== 'deleteConfirm' &&
      e.target.id !== 'deleteAlert'
    ) {
      this.setState({
        isDeleteProfileToggle: false,
      });
  ...
  deleteProfileItem = () => {
    ...
    if (position === 0) {
      nextProfile = position + 1;
    } else {
      nextProfile = position - 1;
    }

    list[nextProfile].choose = 'selected';
  ...
  render() {
    ...
    if (this.state.isDropdownProfilesExpand) {
      profileDropdown = (
        <div
          id="profileDrop"
          className="s3-dropdown expand"
          onClick={this.handleProfileToggle}
        >
          <div className="selected">
            {this.state.profilesItems[this.state.selected].name}
          </div>
      ...
      <div className="profile-bar flex">
        ...
        >
          <div
            ...
            id="profileMenu"
          >
            <div className="act action" onClick={this.addProfileItem}>
              add
            </div>
            <div className="act action">import</div>
            <div className="act divider" />
            <div className="act action" onClick={this.handleRenameToggle}>
              rename
            </div>
            <div className="act action" onClick={this.duplicateProfileItem}>
              duplicate
            </div>
